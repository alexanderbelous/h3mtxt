...
namespace h3m
{
  namespace Detail_NS
  {
    // Type trait for deducing the size of the input EnumBitmask.
    //
    // This is essentially the same as sizeof(T), but, unlike sizeof(), does not
    // require the full definition of EnumBitmask.
    //
    // \param T - input type. If T is an instantiation of EnumBitmask template,
    //        this type trait will have a static member named `value`, storing
    //        the size of the bitmask in bytes.
    //        Otherwise, there will be no such member (the type trait is SFINAE-friendly).
    template<class T>
    struct EnumBitmaskSize {};

    // Partial specialization of EnumBitmaskSize for instantiations of EnumBitmask template.
    ...
    // Helper variable template for EnumBitmaskSize.
    template<class T>
  ...
  struct FieldNames<Tile>
  {
    ...
    static inline constexpr std::string_view kFlags = "flags";
  };
  ...
  struct FieldNames<TownBuildingsBitmask>
  {
    ...
    static inline constexpr std::size_t kNumBits = Detail_NS::EnumBitmaskSize_v<TownBuildingsBitmask> * 8;
