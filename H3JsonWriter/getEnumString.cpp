...
namespace h3m
{
  ...
  {
    // Valid integer values for ArtifactType.
    constexpr std::size_t kNumArtifacts = 144;
    ...
    const std::size_t artifact_type_idx = static_cast<std::size_t>(value);
    // Unknown ArtifactType - return an empty string_view.
    if (artifact_type_idx >= kNumArtifacts)
    {
      return {};
    }
    const h3m::H3JsonWriter_NS::LocalizedStringId string_id =
      static_cast<h3m::H3JsonWriter_NS::LocalizedStringId>(artifact_type_idx);
    return h3m::H3JsonWriter_NS::LocalizedStringsProvider::get()(string_id);
  }
