...
Unlike maps (.h3m) and campaigns (.h3c), the file format for saved games doesn't have a well-established name. Depending on the number of players, the game uses various filename extensions (.CGM, .GM1, .GM2, ..., .GM8), but "GM" is a very ambiguous abbreviation.
...
* Saved campaigns use a different signature "H3SVC", but the file format is the same, so this document uses **H3SVG** for both.
...
Note, however, that the saved games written by HoMM3 have invalid CRC32 checksums at the end of the file, i.e. the bytes [N-8; N-4), where N is
the length of the compressed file in bytes. Because of this, a lot of libraries and file archivers fail to decompress such files. 7-Zip is a notable exception: while it will report an error, it will at least write the decompressed data.
...
Some fields are intentionally left typeless - they always represent meaningless bytes in H3SVG. The size of such such fields is always known in advance (i.e. never varies). You can interpret them as arrays of 8-bit integers, or as whatever you want.
...
An H3SVG file is a single structure which we will naturally call [`SavedGame`](#SavedGame).

### `SavedGame`

| Field name                | Type                             | Size in bytes | Description                                                      |
| ------------------------- | -------------------------------- | ------------- | ---------------------------------------------------------------- |
| file_signature            | `char[5]`                        | 5             | "H3SVG" for standalone maps, "H3SVC" for campaigns               |
...
| victory_condition         | [`VictoryCondition`](#VictoryCondition) | varies | "Map Specifications / Special Victory Condition" in the Editor   |
| loss_condition            | [`LossCondition`](#LossCondition) | varies       | "Map Specifications / Special Loss Condition" in the Editor      |
...
| has_campaign_info         | `Bool`                           | 1             | 0 if this is a standalone map, nonzero otherwise                 |
| ?campaign_info            | [`CampaignInfo`](#CampaignInfo)  | varies        | Campaign-specific data. Only present if `has_campaign_info != 0` |
| num_placeholder_heroes    | `uint8`                          | 1             | The number of Hero Placeholders at the beginning of the scenario |
| placeholder_heroes        | `uint16[num_placeholder_heroes]` | varies        | Hero Placeholders at the beginning of the scenario. Each element is enum `HeroType` |
| original_filename         | `char[47]`                       | 47            | Original filename used for this saved game                       |
| unknown2                  | `uint8[352]`                     | 352           | TODO                                                             |
...
| black_markets             | [`ArtifactMerchants[num_black_markets]`](#ArtifactMerchants) | varies | Current state for each Black Market on the map |
...
| object_properties_tables  | [`ObjectPropertiesTables`](#ObjectPropertiesTables) | varies | Tables storing properties for objects on the Adventure Map |
...
| unknown4                  | `uint8[314]`                     | 314           | TODO                                                             |
| grail_x                   | `int16`                          | 2             | X-coordinate of the Grail, or -1 if there is none                |
| grail_y                   | `int16`                          | 2             | Y-coordinate of the Grail, or -1 if there is none                |
| grail_z                   | `int8`                           | 1             | Z-coordinate of the Grail, or -1 if there is none                |
| unknown4a                 | `uint8[3]`                       | 3             | TODO                                                             |
| is_cheater                | `Bool`                           | 1             | Indicates whether any player has cheated                         |
| day                       | `uint16`                         | 2             | Day of the current date (normally within [1; 7])                 |
| week                      | `uint16`                         | 2             | Week of the current date (normally within [1; 4])                |
| month                     | `uint16`                         | 2             | Month of the current date (initialized with 1 at the start of the game; resets to 0 after Month 65535) |
| unknown5                  | `uint8[32]`                      | 32            | TODO                                                              |
| artifact_merchants        | [`ArtifactMerchants`](#ArtifactMerchants) | 28   | Artifacts currently available in Artifact Merchants              |
| unknown6                  | `uint8[32]`                      | 32            | TODO                                                             |
| keymasters_tents          | `uint8[8]`                       | 8             | 8 bitmasks - 1 per Keymaster's Tent type - indicating which players have visited it |
| unknown7                  | `uint8[6]`                       | 6             | TODO                                                             |
| cartographer_water        | `uint8`                          | 1             | Bitmask; 1 bit per player, indicating if they have visited the Water Cartographer |
| cartographer_land         | `uint8`                          | 1             | Bitmask; 1 bit per player, indicating if they have visited the Land Cartographer |
| cartographer_subterranean | `uint8`                          | 1             | Bitmask; 1 bit per player, indicating if they have visited the Subterranean Cartographer |
| unknown8                  | `uint8[4]`                       | 4             | TODO                                                             |
| fog_of_war                | [`TileVisibility[NUM_TILES]`](#TileVisibility) | varies | Describes visibility of each tile for each player         |
| monoliths_two_way         | [`ObjectExits[8]`](#ObjectExits) | varies        | Exists for each subtype of Monolith Two-Way                      |
| monoliths_one_way         | [`ObjectExits[8]`](#ObjectExits) | varies        | Exists for each subtype of Monolith One-Way                      |
| whirlpools                | [`ObjectExits`](#ObjectExits)    | varies        | All actionable tiles of Whirlpools                               |
| subterranean_gates        | [`ObjectExits`](#ObjectExits)    | varies        | All actionable tiles of Subterranean Gates                       |
| num_unknown9              | `uint16`                         | 2             | The number of elements in `unknown9` field                       |
| unknown9                  | `uint32[num_unknown9]`           | varies        | TODO                                                             |
| num_universities          | `uint16`                         | 2             | The number of Universities on the Adventure Map                  |
| universities              | [`University[num_universities]`](#University) | varies | Universities on the Adventure Map                          |
| num_replay_events         | `uint32`                         | 4             | The number of recorded events from the previous turn             |
| replay_events             | [`ReplayEvent[num_replay_events]`](#ReplayEvent) | varies | Recorded events from the previous turn |
...
| Field name            | Type                                 | Size in bytes | Description                                                       |
| --------------------- | ------------------------------------ | ------------- | ----------------------------------------------------------------- |
| type                  | `uint8`                              | 1             | Enum `VictoryConditionType`                                       |
| ?allow_normal_win     | `Bool`                               | 1             | Only present if type is NOT `0xFF` (`Normal`)                     |
| ?applies_to_computer  | `Bool`                               | 1             | Only present if type is NOT `0xFF` (`Normal`)                     |
| ?artifact             | `uint8`                              | 1             | Only present if type = 0 (`AcquireArtifact`); Enum `ArtifactType` |
| ?accumulate_creatures | [`AccumulateCreatures`](#AccumulateCreatures) | 5    | Only present if type = 1 (`AccumulateCreatures`)                  |
| ?accumulate_resources | [`AccumulateResources`](#AccumulateResources) | 5    | Only present if type = 2 (`AccumulateResources`)                  |
| ?upgrade_town         | [`UpgradeTown`](#UpgradeTown)        | 5             | Only present if type = 3 (`UpgradeTown`)                          |
| ?town_coordinates     | [`Coordinates`](#Coordinates)        | 3             | Only present if type = 4 (`BuildGrail`) or 6 (`CaptureTown`)      |
| ?hero                 | `uint8`                              | 1             | Only present if type = 5 (`DefeatHero`); Enum `HeroType`          |
| ?monster_coordinates  | [`Coordinates`](#Coordinates)        | 3             | Only present if type = 7 (`DefeatMonster`)                        |
| ?transport_artifact   | [`TransportArtifact`](#TransportArtifact) | 4        | Only present if type = 10 (`TransportArtifact`)                   |
| ?days                 | `int32`                              | 4             | Only present if type = 12 (`SurviveBeyondATimeLimit`)             |

Note that victory condition types 8 (`FlagDwellings`), 9 (`FlagMines`) and 11 (`DefeatAllMonsters`) don't have any extra fields
except `allow_normal_win` and `applies_to_computer`.


### `AccumulateCreatures`
| Field name    | Type    | Size in bytes | Description         |
| ------------- | ------- | ------------- | ------------------- |
| creature_type | `uint8` | 1             | Enum `CreatureType` |
| count         | `int32` | 4             |                     |
...
| Field name    | Type    | Size in bytes | Description         |
| ------------- | ------- | ------------- | ------------------- |
| resource_type | `uint8` | 1             | Enum `ResourceType` |
| amount        | `int32` | 4             |                     |
...
| Field name   | Type                          | Size in bytes | Description             |
| ------------ | ----------------------------- | ------------- | ----------------------- |
| artifact     | `uint8`                       | 1             | Enum `ArtifactType`     |
...
| Field name | Type                          | Size in bytes | Description                                            |
| ---------- | ----------------------------- | ------------- | ------------------------------------------------------ |
| type       | `uint8`                       | 1             | Enum `LossConditionType`                               |
| ?town      | [`Coordinates`](#Coordinates) | 3             | Only present if type = 0 (`LoseTown`)                  |
| ?hero      | `uint16`                      | 2             | Only present if type = 1 (`LoseHero`); Enum `HeroType` |
| ?days      | `int16`                       | 2             | Only present if type = 2 (`TimeExpires`)               |
...
| map_filename         | `char[251]`          | 251           | The original filename of the map. Only the characters before the first null terminator are significant. Used by "Restart Scenario" command. For campaign scenarios it's always "test.h3m" |
| map_directory        | `char[100]`          | 100           | Relative (to Heroes3.exe) path to the directory in which the original map file is located. Only the characters before the first null terminator are significant. Used by "Restart Scenario" command. The game always writes "maps" here for standalone maps |
| players_control      | `uint8[8]`           | 8             | Enum `PlayerControlType` for each player                               |
| unknown2             | `uint8[3]`           | 3             | Unknown. Seems to always be [255, 1, 1]                                |
| player_turn_duration | `uint8`              | 1             | Enum `PlayerTurnDuration`. Meaningless for campaigns                   |
| starting_heroes      | `uint8[8]`           | 8             | Enum `HeroType` for each player. Meaningless for campaigns             |
| starting_bonuses     | `uint8[8]`           | 8             | Enum `PlayerStartingBonus` for each player. Meaningless for campaigns  |


### `CampaignInfo`
| Field name          | Type                              | Size in bytes | Description                                                |
| ------------------- | --------------------------------- | ------------- | -----------------------------------------------------------|
| unknown1            | `uint8[3]`                        | 3             | TODO                                                       |
| region_idx          | `uint8`                           | 1             | 0-based index of the current region                        |
| id                  | `uint8`                           | 1             | Enum `CampaignId`                                          |
| unknown2            | `uint8[2]`                        | 2             | TODO                                                       |
| starting_bonus_idx  | `uint8`                           | 1             | 0-based index of the selected starting bonus               |
| filename            | `String32`                        | varies        | The original filename of the .h3c file                     |
| finished_campaigns  | `Bool[21]`                        | 21            | 1 `Bool` per enum `CampaignId`, indicating which campaigns have been finished |
| num_regions         | `uint8`                           | 1             | The number of regions in this campaign                     |
| regions             | [`RegionInfo[num_regions]`](#RegionInfo) | varies | Information for each region in this campaign               |
| num_crossover_infos | `uint8`                           | 1             | The number of elements in `crossover_info`                 |
| crossover_info      | [`CrossoverInfo[num_crossover_infos]`](#CrossoverInfo) | varies | Information about crossover heroes           |


### `RegionInfo`
| Field name   | Type     | Size in bytes | Description                                                                                          |
| ------------ | -------- | ------------- | ---------------------------------------------------------------------------------------------------- |
| is_completed | `Bool`   | 1             | 0 if this scenario hasn't been completed yet, nonzero otherwise                                      |
| days         | `uint32` | 4             | The number of days that it took the player to complete this scenario                                 |
| score        | `uint32` | 4             | Score for this scenario (see https://heroes.thelazy.net/index.php/Score)                             |
| order        | `uint8`  | 1             | 1-based rank of this scenario among all scenarios in the order in which they were played (ascending) |
| unknown      | `uint8`  | 1             | TODO                                                                                                 |


### `CrossoverInfo`
| Field name  | Type                        | Size in bytes | Description
| ----------- | --------------------------- | ------------- | ------------------------------------------------------------------------------------ |
| num_heroes  | `uint8`                     | 1             | The number of crossover heroes                                                       |
| heroes      | [`Hero[num_heroes]`](#Hero) | varies        | Settings for each crossover hero                                                     |
| num_unknown | `uint16`                    | 2             | The number of elements in `unknown` field                                            |
| unknown     | `uint32[num_unknown]`       | varies        | TODO. Each element is actually a pair of 16-bit integers, but the meaning is unclear |
...
| Field name            | Type       | Size in bytes | Description                                                                        |
| --------------------- | ---------- | ------------- | ---------------------------------------------------------------------------------- |
| terrain_type          | `uint8`    | 1             | Enum `TerrainType`                                                                 |
...
| Field name         | Type        | Size in bytes | Description                                                  |
| ------------------ | ----------- | ------------- | ------------------------------------------------------------ |
| exists             | `Bool`      | 1             | 0 if scuttled/sunk, nonzero otherwise                        |
| id                 | `uint8`     | 1             | 0-based index of this boat in `ObjectPropertiesTables.boats` |
| object_subclass    | `uint8`     | 1             | Enum `BoatType`                                              |
| direction          | `uint8`     | 1             | Enum `CompassPoint`; the current orientation of the boat     |
| owner              | `uint8`     | 1             | Enum `PlayerColor`                                           |
| owner_hero         | `uint8`     | 1             | Enum `HeroType`                                              |
| is_occupied        | `Bool`      | 1             | 0 if the boat is boarded by a hero, nonzero otherwise        |
| x                  | `uint16`    | 2             | X-coordinate of the boat on the Adventure Map                |
| y                  | `uint16`    | 2             | Y-coordinate of the boat on the Adventure Map                |
| z                  | `uint16`    | 2             | Z-coordinate of the boat on the Adventure Map                |
| is_visible         | `Bool`      | 1             | 0 if the boat is occupied/scuttled/sunk, nonzero otherwise   |
| coordinates_packed | `uint32`    | 4             | Duplicates the coordinates using the packed format           |
| unknown            | `uint8[9]`  | 9             | TODO                                                         |
...
| unknown1         | `uint8`     | 1             | TODO                                                                                    |
| personality      | `uint32`    | 4             | Enum `PlayerPersonality`                                                                |
| unknown2         | `uint8[5]`  | 5             | TODO                                                                                    |
...
| towns            | `int8[72]`  | 72            | IDs of towns owned by the player (see Town.id)                                          |
| resources        | `int32[7]`  | 28            | The current amount for each Resource type                                               |
| mystical_gardens | `uint8[4]`  | 4             | Bitmask; 1 bit per Mystical Garden, indicating if the player has visited it this week   |
| magic_springs    | `uint8[4]`  | 4             | Bitmask; 1 bit per Magic Spring, indicating if the player has visited it this week      |
| corpses          | `uint8[4]`  | 4             | Bitmask; 1 bit per Corpse, indicating if the player has visited it                      |
| lean_tos         | `uint8[4]`  | 4             | Bitmask; 1 bit per Lean To, indicating if the player has visited it                     |
| unknown3         | `uint8[3]`  | 3             | TODO                                                                                    |
...
| num_external_dwellings | `uint32[7]`                 | 28            | The number of external dwellings for each creature level                  |
| num_external_dwellings2 | `uint32[7]`                | 28            | Duplicates num_external_dwellings                                         |
| unknown3             | `uint8[13]`                   | 13            | |
| built                | `uint8[6]`                    | 6             | Bitmask; 1 bit per `TownBuildingTypeH3SVG`, indicating if it's built      |
| reserved1            |                               | 2             | Not used                                                                  |
| can_be_built         | `uint8[6]`                    | 6             | Bitmask; 1 bit per `TownBuildingTypeH3SVG`, indicating if it can be built |
| reserved2            |                               | 2             | Not used                                                                  |
| spells               | `int32[30]`                   | 120           | 6 spells (enum `SpellType`) for each level of the Mage Guild              |
| banned_spells        | `uint8[9]`                    | 9             | Bitmask; 1 bit per spell type, indicating if it is banned                 |
| unknown4             | `uint8[61]`                   | 61            | |
| unique_buildings_properties | [`UniqueBuildingsProperties`](#UniqueBuildingsProperties) | 7 | Properties of town-specific buildings              |


### `UniqueBuildingsProperties`
| Field name              | Type  | Size in bytes | Description                                                            |
| ----------------------- | ----- | ------------- | ---------------------------------------------------------------------- |
| packed_data             | uint8 | 1             | Bit 0 (LSB) - `mana_vortext_available` (`Bool`); Bits [1; 3] - `mystic_pond_resource_amount` (`uint8`); Bits [4; 6] - `mystic_pond_resource_type` (enum `ResourceType`); Bit 7 (MSB) - unknown |
| summoned_creature_type  | int32 | 4             | Enum `CreatureType` or -1 if there is no summoned creature             |
| summoned_creature_count | int16 | 2             | The number of creatures in the Portal of Summoning                     |
...
| unknown1                   | `uint8[5]`  | 5             | TODO                                                                              |
| is_female                  | `Bool`      | 1             | 1 if the hero is female, 0 otherwise                                              |
| use_custom_biography       | `Bool`      | 1             | 1 if the custom biography should be displayed, 0 otherwise                        |
| biography                  | `String32`  | varies        | The hero's biography                                                              |
| owner                      | `uint8`     | 1             | Enum `PlayerColor`                                                                |
| patrol_radius              | `int8`      | 1             | Patrol radius or -1 if the hero has no patrol                                     |
| temp_morale                | `int8`      | 1             | Temporary Morale bonus until the next battle                                      |
| temp_luck                  | `int8`      | 1             | Temporary Luck bonus until the next battle                                        |
| backpack_count             | `uint8`     | 1             | The number of artifacts in the backpack                                           |
| disguise_level             | `int8`      | 1             | (Air Magic) Level at which Disguise was cast, or -1 if it hasn't been cast        |
| fly_level                  | `int8`      | 1             | (Air Magic) Level at which Fly was cast, or -1 if it hasn't been cast             |
| num_dimension_door_casts   | `uint8`     | 1             | The number of times Dimension Door was cast                                       |
| water_walk_level           | `int8`      | 1             | (Water Magic) Level at which Water Walk was cast, or -1 if it hasn't been cast    |
| visions_level              | `int8`      | 1             | Level at which Visions was cast, or -1 if it hasn't been cast                     |
| hero_type                  | `uint8`     | 1             | Enum `HeroType`                                                                   |
| hero_class                 | `uint8`     | 1             | Enum `HeroClass`                                                                  |
| hero_portrait              | `uint8`     | 1             | Enum `HeroPortrait`                                                               |
| patrol_x                   | `uint8`     | 1             | X-coordinate of the starting position if the hero has patrol; 0xFF otherwise      |
| patrol_y                   | `uint8`     | 1             | Y-coordinate of the starting position if the hero has patrol; 0xFF otherwise      |
| unknown2                   | `uint8[4]`  | 4             | TODO                                                                              |
| destination_x              | `int32`     | 4             | X-coordinate of the destination, or -1 if there is none                           |
| destination_y              | `int32`     | 4             | Y-coordinate of the destination, or -1 if there is none                           |
| destination_z              | `int8`      | 1             | Z-coordinate of the destination                                                   |
| unknown3                   | `uint8[3]`  | 3             | TODO                                                                              |
| move_points_max            | `int32`     | 4             | Maximum number of movement points at the moment                                   |
| move_points                | `int32`     | 4             | The actual number of movement points at the moment                                |
| experience                 | `int32`     | 4             | Experience points                                                                 |
| unknown4                   | `uint8[4]`  | 4             | TODO                                                                              |
| spell_points               | `uint16`    | 2             | Spell points                                                                      |
| level                      | `int16`     | 2             | Experience level                                                                  |
| unknown5                   | `uint8[2]`  | 2             | TODO                                                                              |
| learning_stones            | `uint8[4]`  | 4             | Bitmask; 1 bit per Learning Stone, indicating if the hero has visited it          |
| marletto_towers            | `uint8[4]`  | 4             | Bitmask; 1 bit per Marletto Tower, indicating if the hero has visited it          |
| gardens_of_revelation      | `uint8[4]`  | 4             | Bitmask; 1 bit per Garden of Revelation, indicating if the hero has visited it    |
| mercenary_camps            | `uint8[4]`  | 4             | Bitmask; 1 bit per Mercenary Camp, indicating if the hero has visited it          |
| star_axes                  | `uint8[4]`  | 4             | Bitmask; 1 bit per Star Axis, indicating if the hero has visited it               |
| trees_of_knowledge         | `uint8[4]`  | 4             | Bitmask; 1 bit per Tree of Knowledge, indicating if the hero has visited it       |
| libraries_of_enlightenment | `uint8[4]`  | 4             | Bitmask; 1 bit per Library of Enlighenment, indicating if the hero has visited it |
| arenas                     | `uint8[4]`  | 4             | Bitmask; 1 bit per Arena, indicating if the hero has visited it                   |
| schools_of_magic           | `uint8[4]`  | 4             | Bitmask; 1 bit per School of Magic, indicating if the hero has visited it         |
| schools_of_war             | `uint8[4]`  | 4             | Bitmask; 1 bit per School of War, indicating if the hero has visited it           |
| reserved                   |             | 16            | Always 0s by default                                                              |
| flags                      | `uint8[4]`  | 4             | Bitmask; 1 bit per enum `HeroFlag`                                                |
...
| artifacts                  | [`HeroArtifacts`](#HeroArtifacts) | 679 |                                                                       |
| is_sleeping                | `Bool`      | 1             | 0 if the hero is awake, nonzero if the hero is sleeping                           |
| visited_towns              | `uint8[6]`  | 6             | Bitmask; 1 bit per town ID, indicating if the hero has visited it                 |


### `HeroArtifacts`
| Field name | Type                      | Size in bytes | Description                                                                 |
| ---------- | ------------------------- | ------------- | --------------------------------------------------------------------------- |
| equipped   | [`HeroArtifact[19]`](#HeroArtifact) | 152 | Artifact in each slot                                                       |
| backpack   | [`HeroArtifact[64]`](#HeroArtifact) | 512 | Artifacts in the backpack                                                   |
| unknown    | `uint8`                   | 1             | TODO                                                                        |
| locks      | `uint8[14]`               | 14            | The number of locks from combination artifacts per enum `ArtifactSlotGroup` |
...
| Field name           | Type    | Size in bytes | Description                                                                    |
| -------------------- | ------- | ------------- | ------------------------------------------------------------------------------ |
| visibility           | `uint8` | 1             | Bitmask; 1 bit per `PlayerColor`, indicating if this tile is currently visible |
| has_adjacent_monster | `Bool`  | 1             | 1 if there's a monster on this or adjacent tile, 0 otherwise                   |


### `ObjectExits`
| Field name | Type                | Size in bytes | Description                                                        |
| ---------- | ------------------- | ------------- | ------------------------------------------------------------------ |
| num_exits  | `uint16`            | 2             | The number of exits                                                |
| exits      | `uint32[num_exits]` | varies        | The coordinates (serialized using the packed format) for each exit |


### `University`
| Field name | Type        | Size in bytes | Description                                                                                         |
| ---------- | ----------- | ------------- | --------------------------------------------------------------------------------------------------- |
| skills     | `uint32[4]` | 16            | Secondarky skills that can be learned in this University. Each element is enum `SecondarySkillType` |


### `ReplayEvent`
| Field name | Type                          | Size in bytes | Description                                            |
| ---------- | ----------------------------- | ------------- | ------------------------------------------------------ |
| type       | `uint8`                       | 1             | Enum `ReplayEventType`                                 |
| details    | varies                        | varies        | TODO                                                   |
